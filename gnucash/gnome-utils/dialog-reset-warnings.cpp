/***********************************************************************
 * dialog-reset-warnings.cpp -- "Resert Warnings" dialog                 *
                              ...
                              GtkDialog *dialog)
{
    ...
    const gchar *pref = gtk_widget_get_name(widget);
    auto prefs_group = static_cast<const gchar *>(
        g_object_get_data (G_OBJECT (widget), "prefs-group")
    );
    if (prefs_group)
