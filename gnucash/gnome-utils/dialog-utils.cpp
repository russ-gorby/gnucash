/********************************************************************\
 * dialog-utils.cpp -- utility functions for creating dialogs         *
 *                   for GnuCash                                    *
...
gnc_gdate_in_valid_range (GDate *test_date, gboolean warn) noexcept
{
    ...
    if (use_autoreadonly)
        ...
        min_date = g_date_new_dmy (1,static_cast<GDateMonth>(1),1400);
                             ...
                             const char *date_str) noexcept
{
    ...
    if (!g_date_valid_dmy (tm->tm_mday, static_cast<GDateMonth>(tm->tm_mon + 1),
                           tm->tm_year + 1900))
        return FALSE;
    ...
    case GDK_KEY_y:
        /* beginning of year */
        g_date_set_day (&gdate, 1);
        g_date_set_month (&gdate, static_cast<GDateMonth>(1));
        break;
...
gnc_perm_button_cb (GtkButton *perm, gpointer user_data)
{
    ...
    gtk_widget_set_sensitive(static_cast<GtkWidget *>(user_data), !perm_active);
}
