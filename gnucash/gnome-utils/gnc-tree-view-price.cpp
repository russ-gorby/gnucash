/********************************************************************\
 * gnc-tree-view-price.cpp -- GtkTreeView implementation to display   *
 ...
 *                                                                  *
 * Free Software Foundation           git status
 * git Voice:  +1-617-542-5942       *
...
{
    ENTER("view %p", widget);
    gnc_leave_return_if_fail (widget != nullptr);
                         ...
                         ...)
{
    ...
    GNCPriceDB *price_db = gnc_pricedb_get_db(book);
    auto model = static_cast<GtkTreeModel *>(
        gnc_tree_model_price_new (book, price_db)
    );
    ...
    auto view = static_cast<GncTreeView *>(
        g_object_new (GNC_TYPE_TREE_VIEW_PRICE, "name", "gnc-id-price-tree", nullptr));
    ...
    const gchar *sample_text = gnc_commodity_get_printname(gnc_default_currency());
    gchar *sample_text2 = g_strdup_printf("%s%s", sample_text, sample_text);
    gnc_tree_view_add_text_column (
              view, _("Security"), "security", nullptr, sample_text2,
              GNC_TREE_MODEL_PRICE_COL_COMMODITY,
              GNC_TREE_VIEW_COLUMN_VISIBLE_ALWAYS,
              sort_by_name);
    g_free(sample_text2);
    GtkTreeViewColumn *col = gnc_tree_view_add_text_column (
              view, _("Currency"), "currency", nullptr, sample_text,
              GNC_TREE_MODEL_PRICE_COL_CURRENCY,
              GNC_TREE_MODEL_PRICE_COL_VISIBILITY,
              sort_by_name);
    g_object_set_data(G_OBJECT(col), DEFAULT_VISIBLE, GINT_TO_POINTER(1));
    col = gnc_tree_view_add_text_column (
              view, _("Date"), "date", nullptr, "2005-05-20",
              GNC_TREE_MODEL_PRICE_COL_DATE,
              GNC_TREE_MODEL_PRICE_COL_VISIBILITY,
              sort_by_date);
    g_object_set_data(G_OBJECT(col), DEFAULT_VISIBLE, GINT_TO_POINTER(1));
    col = gnc_tree_view_add_text_column (
              view, _("Source"), "source", nullptr, "Finance::Quote",
              GNC_TREE_MODEL_PRICE_COL_SOURCE,
              GNC_TREE_MODEL_PRICE_COL_VISIBILITY,
              sort_by_source);
    g_object_set_data(G_OBJECT(col), DEFAULT_VISIBLE, GINT_TO_POINTER(1));
    col = gnc_tree_view_add_text_column (
              view, _("Type"), "type", nullptr, "last",
                                   ...
                                   gpointer data)
{
    ...
    if (gnc_tree_model_price_iter_is_namespace (GNC_TREE_MODEL_PRICE(model), iter))
    {
        if (fd->user_ns_fn)
        {
            gnc_commodity_namespace *name_space = gnc_tree_model_price_get_namespace (GNC_TREE_MODEL_PRICE(model), iter);
    ...
    if (gnc_tree_model_price_iter_is_commodity (GNC_TREE_MODEL_PRICE(model), iter))
    {
        if (fd->user_cm_fn)
        {
            gnc_commodity *commodity = gnc_tree_model_price_get_commodity (GNC_TREE_MODEL_PRICE(model), iter);
    ...
    if (gnc_tree_model_price_iter_is_price (GNC_TREE_MODEL_PRICE(model), iter))
    {
        if (fd->user_pc_fn)
        {
            GNCPrice *price = gnc_tree_model_price_get_price (GNC_TREE_MODEL_PRICE(model), iter);
                                ...
                                GDestroyNotify destroy)
{
    ...
    auto fd = static_cast<filter_user_data *>(
        g_malloc(sizeof(filter_user_data))
    );
...
gnc_tree_view_price_get_selected_price (GncTreeViewPrice *view)
{
    ...
    ENTER("view %p", view);
    g_return_val_if_fail (GNC_IS_TREE_VIEW_PRICE (view), nullptr);
...
get_selected_prices_helper (GtkTreeModel *s_model,
                            ...
                            gpointer data)
{
    auto return_list = static_cast<GList **>(data);
    GtkTreeIter iter, f_iter;
    ...
    GNCPrice *price = gnc_tree_model_price_get_price (GNC_TREE_MODEL_PRICE(model),
                                            &iter);
    if (price)
