/*
 * dialog-query-view.cpp -- a simple dialog to display a query view and
...
dqv_build_booklist (DialogQueryView *dqv, Query *q)
{
    ...
    {
        auto book = static_cast<QofBook *>(node->data);
...
{
    g_return_if_fail (dqv);
    gpointer entry = gnc_query_view_get_selected_entry (GNC_QUERY_VIEW (dqv->qview));
    if (!entry)
        return;

    auto cb = static_cast<GNCDisplayViewButton *>(
...
gnc_dialog_query_view_double_click_entry (GNCQueryView *qview, gpointer item,
                                          gpointer user_data)
{
    auto dqv = static_cast<DialogQueryView *>(user_data);
