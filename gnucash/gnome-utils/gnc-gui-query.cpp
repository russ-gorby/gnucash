/********************************************************************\
 * gnc-gui-query.cpp -- functions for creating dialogs for GnuCash    *
                     ...
                     const gchar *format, ...) noexcept
{
    ...
    if (!parent)
        parent = gnc_ui_get_main_window (nullptr);
...
gnc_info2_dialog (GtkWidget *parent, const gchar *title, const gchar *msg) noexcept
{
    ...
    GtkWidget* dialog = gtk_dialog_new_with_buttons (
        title, GTK_WINDOW (parent), flags, _("_OK"), GTK_RESPONSE_ACCEPT, nullptr);
    GtkWidget* content_area = gtk_dialog_get_content_area (GTK_DIALOG (dialog));
