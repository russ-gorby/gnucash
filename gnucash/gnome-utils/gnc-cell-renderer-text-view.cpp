                    ...
                    GncCellRendererTextView *cell_tv)
{
    ...
    auto path = static_cast<gchar *>(
        g_object_get_data (G_OBJECT(editable), GNC_CELL_RENDERER_TEXT_VIEW_PATH)
    );

    gchar *new_text = gnc_cell_view_get_text (GNC_CELL_VIEW(editable));
                     ...
                     GtkCellRendererState  flags)
{
    ...
    g_object_add_weak_pointer (G_OBJECT(cell_tv->editable),
                               reinterpret_cast<gpointer *>(&cell_tv->editable));
