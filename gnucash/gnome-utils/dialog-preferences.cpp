/********************************************************************\
 * dialog-preferences.cpp -- preferences dialog                       *
 *                                                                  *
...
{
    gchar *separator = nullptr;
    gchar *conflict_msg = gnc_account_separator_is_valid (gtk_entry_get_text (entry), &separator);

    auto label = static_cast<GtkWidget *>(
        g_object_get_data (G_OBJECT(dialog), "sample_account")
    );
    ...
    /* Translators: Both %s will be the account separator character; the
       ...
       translation. */
    gchar *sample = g_strdup_printf (_("Income%sSalary%sTaxable"),
                              separator, separator);
    ...
    auto image = static_cast<GtkWidget *>(
        g_object_get_data (G_OBJECT(dialog), "separator_error")
    );
...
gnc_account_separator_validate (GtkWidget *dialog)
{
    auto entry = static_cast<GtkWidget *>(
        g_object_get_data (G_OBJECT(dialog), "account-separator")
    );
    gboolean ret = TRUE;
    gchar *separator = nullptr;
    ...
    if (conflict_msg)
    {
        ...
        {
            auto original_sep = static_cast<gchar *>(
                g_object_get_data (G_OBJECT(entry), "original_text")
            );

            if (original_sep != nullptr)
...
gnc_preferences_add_page_internal (const gchar *filename,
                                   ...
                                   gboolean full_page)
{
    addition *preexisting = nullptr;
    bool error = false;

    ENTER("file %s, widget %s, tab %s full page %d",
          filename, widgetname, tabname, full_page);

    auto add_in = static_cast<addition *>(g_malloc (sizeof(addition)));
    if (add_in == nullptr)
    {
    ...
    GSList *ptr = g_slist_find_custom (add_ins, add_in, (GCompareFunc)gnc_prefs_compare_addins);
    if (ptr)
    {
        /* problem? */
        preexisting = static_cast<addition *>(ptr->data);
...
gnc_prefs_build_widget_table (GtkBuilder *builder,
                              GtkWidget *dialog)
{
    auto prefs_table = static_cast<GHashTable *>(
        g_object_get_data (G_OBJECT(dialog), PREFS_WIDGET_HASH)
    );
    GSList *interesting = gtk_builder_get_objects (builder);

    for (GSList *runner = interesting; runner; runner = g_slist_next(runner))
    {
        auto widget = static_cast<GtkWidget *>(runner->data);
        if (GTK_IS_WIDGET(widget))
        {
            const gchar *wname = gtk_widget_get_name (widget);
...
gnc_prefs_find_page (GtkNotebook *notebook, const gchar *name)
{
    ...
    g_return_val_if_fail (GTK_IS_NOTEBOOK(notebook), nullptr);
    ...
    for (i = 0; i < n_pages; i++)
    {
        ...
        g_return_val_if_fail (child, nullptr);

        child_name = gtk_notebook_get_tab_label_text (notebook, child);
        g_return_val_if_fail (child_name, nullptr);
                            ...
                            gpointer data)
{
    ...
    gtk_container_child_get (GTK_CONTAINER(copydata->grid_from), child,
                             ...
                             "width", &width,
                             nullptr);
    bool hexpand = gtk_widget_get_hexpand (child);
    bool vexpand = gtk_widget_get_vexpand (child);
    GtkAlign halign = gtk_widget_get_halign (child);
    GtkAlign valign = gtk_widget_get_valign (child);

    g_object_get (child, "margin-top", &topm, "margin-bottom", &bottomm, nullptr);
    g_object_get (child, "margin-left", &leftm, "margin-right", &rightm, nullptr);
    ...
    g_object_set (child, "margin-left", leftm, "margin-right", rightm, nullptr);
    g_object_set (child, "margin-top", topm, "margin-bottom", bottomm, nullptr);
...
for (i = 0; widgetname[i]; i++)
    ...
    GtkWidget *new_content = GTK_WIDGET(gtk_builder_get_object (builder, widgetname[i - 1]));
    ...
    auto notebook = static_cast<GtkNotebook *>(
        g_object_get_data (G_OBJECT(dialog), NOTEBOOK)
    );
    ...
    GtkWidget *existing_content = gnc_prefs_find_page (notebook, add_in->tabname);
...
{
    gint n_pages, i;
    GList *tabs = nullptr, *iter = nullptr;
...
gnc_prefs_split_widget_name (const gchar *name, gchar **group, gchar **pref, gchar **value)
{
    ...
    *pref = g_strdup (value_splits[0]);
    if (value)
        *value = g_strdup (value_splits[1]); /* may be nullptr */
    g_strfreev (splits);
...
gnc_prefs_connect_font_button (GtkFontButton *fb)
{
    ...
    gnc_prefs_split_widget_name (gtk_buildable_get_name (GTK_BUILDABLE(fb)), &group, &pref, nullptr);
    gnc_prefs_bind (group, pref, nullptr, G_OBJECT (fb), "font-name");
...
{
    auto image = static_cast<GtkImage *>(
        g_object_get_data (G_OBJECT(fc), "path_head_error")
    );
    auto group = static_cast<const gchar *>(
        g_object_get_data (G_OBJECT(fc), "group")
    );
    auto pref = static_cast<const gchar *>(
        g_object_get_data (G_OBJECT(fc), "pref")
    );
    ...
    {
        gchar *folder_with_slash = g_strconcat (folder_uri, "/", nullptr);
        g_free (folder_uri);
...
{
    gchar *group = nullptr, *pref = nullptr;
    bool folder_set = true;

    g_return_if_fail (GTK_FILE_CHOOSER_BUTTON(fcb));

    if (boxname == nullptr)
        gnc_prefs_split_widget_name (gtk_buildable_get_name (GTK_BUILDABLE(fcb)), &group, &pref, nullptr);
    else
        gnc_prefs_split_widget_name (boxname, &group, &pref, nullptr);

    gchar *uri = gnc_prefs_get_string (group, pref);
    ...
    auto image = static_cast<GtkImage *>(
        g_object_get_data (G_OBJECT(fcb), "path_head_error")
    );
    ...
    {
        gchar file_str[] = "file";
        gchar *path_head = gnc_doclink_get_unescape_uri (nullptr, uri, file_str);
        gchar *ttip = g_strconcat (_("Path does not exist, "), path_head, nullptr);
    ...
    g_signal_connect (GTK_FILE_CHOOSER(fcb), "selection-changed",
                      G_CALLBACK(file_chooser_selected_cb), nullptr);
...
file_chooser_clear_cb (GtkButton *button, gpointer user_data)
{
    ...
    auto group = static_cast<const gchar *>(
        g_object_get_data (G_OBJECT(fcb), "group")
    );
    auto pref = static_cast<const gchar *>(
        g_object_get_data (G_OBJECT(fcb), "pref")
    );
    auto image = static_cast<GtkImage *>(
        g_object_get_data (G_OBJECT(fcb), "path_head_error")
    );
    ...
    GtkWidget *box = gtk_widget_get_parent (GTK_WIDGET(fcb));
    g_signal_handlers_disconnect_by_func (button,
        reinterpret_cast<gpointer>(file_chooser_clear_cb), fcb);
    ...
    GtkWidget *fcb_new = gtk_file_chooser_button_new (_("Select a folder"),
    ...
    gchar *boxname = g_strconcat ("pref/", group, "/", pref, nullptr);
...
gnc_prefs_connect_check_button (GtkCheckButton *button)
{
    ...
    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(button), "active");
...
gnc_prefs_connect_spin_button (GtkSpinButton *spin)
{
    ...
    gnc_prefs_split_widget_name (gtk_buildable_get_name (GTK_BUILDABLE(spin)), &group, &pref, nullptr);

    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(spin), "value");
...
gnc_prefs_connect_combo_box (GtkComboBox *box)
{
    ...
    gnc_prefs_split_widget_name (gtk_buildable_get_name (GTK_BUILDABLE(box)), &group, &pref, nullptr);

    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(box), "active");
...
gnc_prefs_connect_currency_edit (GNCCurrencyEdit *gce, const gchar *boxname )
{
    ...
    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(gce), "mnemonic");
...
gnc_prefs_connect_entry (GtkEntry *entry)
{
    ...
    gnc_prefs_split_widget_name (gtk_buildable_get_name (GTK_BUILDABLE(entry)), &group, &pref, nullptr);

    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(entry), "text");
...
gnc_prefs_connect_period_select (GncPeriodSelect *period, const gchar *boxname )
{
    ...
    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(period), "active");
...
gnc_prefs_connect_date_edit (GNCDateEdit *gde , const gchar *boxname )
{
    ...
    gnc_prefs_bind (group, pref, nullptr, G_OBJECT(gde), "time");
                       ...
                       gpointer user_data)
{
    ...
    else if (GTK_IS_FILE_CHOOSER_BUTTON(widget))
    {
        ...
        gnc_prefs_connect_file_chooser_button (GTK_FILE_CHOOSER_BUTTON(widget), nullptr);
    }
    ...
    else if (GTK_IS_BOX(widget))
    {
        ...
        auto widget_child = static_cast<GtkWidget *>(child->data);
        g_list_free (child);
        DEBUG("  %s - box", name);
...
gnc_preferences_dialog_create (GtkWindow *parent)
{
    ...
    date = gnc_date_edit_new (gnc_time (nullptr), FALSE, FALSE);
