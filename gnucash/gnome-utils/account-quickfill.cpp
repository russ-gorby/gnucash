...
shared_quickfill_destroy (QofBook* book, gpointer key, gpointer user_data)
{
    ...
    gnc_prefs_remove_cb_by_func (GNC_PREFS_GROUP_GENERAL,
                                 ...
                                 reinterpret_cast<gpointer>(shared_quickfill_pref_changed),
                                 qfb);
                                ...
                                gpointer user_data)
{
    ...
    for (GList *tmp = data->accounts; tmp; tmp = g_list_next (tmp))
    {
        if (tmp->data == account)
        {
                                       ...
                                       AccountBoolCB cb, gpointer cb_data) noexcept
{
    ...
    qfb = static_cast<QFB *>(
        build_shared_quickfill (book, root, key, cb, cb_data)
    );
    return qfb->qf;
}
                           ...
                           gpointer user_data, gpointer event_data)
{
    ...
    if (!GNC_IS_ACCOUNT (entity))
        return;
    Account *account = GNC_ACCOUNT (entity);
    ...
    case QOF_EVENT_MODIFY:
        ...
        {
            account = static_cast<Account *>(tmp->data);
            if (qfb->dont_add_cb)
            {
