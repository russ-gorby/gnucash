/*
 * dialog-tax-table.cpp -- Dialog to create and edit tax-tables
...
new_tax_table_ok_cb (NewTaxTable *ntt)
{
    ...
    /* Create/edit the entry */
    {
        ...
        gncTaxTableEntrySetType (entry, static_cast<GncAmountType>(ntt->type));
...
tax_table_entries_refresh (TaxTableWindow *ttw)
{
    ...
    GtkTreeView *view = GTK_TREE_VIEW(ttw->entries_view);
    ...
    GList *list = gncTaxTableGetEntries (ttw->current_table);
    if (list)
...
tax_table_window_refresh (TaxTableWindow *ttw)
{
    ...
    g_return_if_fail (ttw);
    GtkTreeView *view = GTK_TREE_VIEW(ttw->names_view);
    GtkListStore *store = GTK_LIST_STORE(gtk_tree_view_get_model (view));
    ...
    /* Add the items to the list */
    GList *list = gncTaxTableGetTables (ttw->book);
    if (list)
        list = g_list_reverse (g_list_copy (list));

    for (GList *node = list; node; node = node->next)
    {
        auto table = static_cast<GncTaxTable *>(node->data);
...
gnc_ui_tax_table_window_new (GtkWindow *parent, QofBook *book) noexcept
{
     ...
     * the window.
     */
    auto ttw = static_cast<TaxTableWindow *>(
        gnc_find_first_gui_component (DIALOG_TAX_TABLE_CM_CLASS, find_handler, book)
    );
    if (ttw)
    {
    ...
    GtkTreeView *view = GTK_TREE_VIEW(ttw->names_view);
    GtkListStore *store = gtk_list_store_new (NUM_TAX_TABLE_COLS, G_TYPE_STRING,
                                G_TYPE_POINTER);
