/********************************************************************\
 * gnc-dense-cal.cpp : a custom densely-dispalyed calendar widget     *
...
month_name (int mon)
{
    ...
    g_date_set_month (&date, static_cast<GDateMonth>(mon + 1));
...
gnc_dense_cal_init (GncDenseCal *dcal)
{
    ...
    if (first_day > 0 && first_day < 8)
        dcal->day_of_week_start = static_cast<GDateWeekday>(first_day);
...
gnc_dense_cal_set_num_months (GncDenseCal *dcal, guint num_months) noexcept
{
    ...
    // set iter on view
    auto cbptr = reinterpret_cast<gpointer>(_gdc_view_option_changed);
    g_signal_handlers_block_by_func (dcal->view_options, cbptr, dcal);
    gtk_combo_box_set_active_iter (GTK_COMBO_BOX(dcal->view_options), &iter_closest_to_req);
    g_signal_handlers_unblock_by_func (dcal->view_options, cbptr, dcal);
...
gdc_free_all_mark_data (GncDenseCal *dcal)
{
    ...
    {
        auto mark = static_cast<gdc_mark_data *>(l->data);
...
get_week_of_year (GncDenseCal *dcal, GDate *d)
{
    ...
    g_date_set_dmy (&first, 1, static_cast<GDateMonth>(1), year);
    ...
    g_date_clear (&last, 1);
    g_date_set_dmy (&last, 31, static_cast<GDateMonth>(12), year - 1);
...
get_weeks_in_year (GncDenseCal *dcal, GDateYear year)
{
    ...
    g_date_clear (&d, 1);
    g_date_set_dmy (&d, 1, static_cast<GDateMonth>(1), year);
    if (g_date_get_weekday (&d) == dcal->day_of_week_start) return 53;
    g_date_set_dmy (&d, 31, static_cast<GDateMonth>(12), year);
    if (g_date_get_weekday (&d) == dcal->day_of_week_start) return 53;
    if (g_date_is_leap_year (year))
    {
        g_date_set_dmy (&d, 2, static_cast<GDateMonth>(1), year);
        if (g_date_get_weekday (&d) == dcal->day_of_week_start) return 53;
        g_date_set_dmy (&d, 30, static_cast<GDateMonth>(12), year);
        if (g_date_get_weekday (&d) == dcal->day_of_week_start) return 53;
    }
    return 52;
}
        ...
        for (doc = 0; g_date_get_julian (&d) < g_date_get_julian (&eoc); g_date_add_days (&d, 1), doc++)
        {
            ...
            if (!today_found && g_date_compare (&d, &now) == 0)
            {
                ...
                if (border.left + border.right != 0)
                {
                    ...
                    const gchar *frame_css = static_cast<const gchar *>(
                        ".marker-border {\n  border-color:black;\n}\n"
                    );
...
int num_weeks_per_col (GncDenseCal *dcal)
{
    ...
    for (i = 0; i < numCols; i++)
    {
        auto gdm = static_cast<GDateMonth>((dcal->month - 1 + (i * dcal->monthsPerCol)) % 12 + 1);
        auto gdy = dcal->year + floor ((dcal->month - 1 + (i * dcal->monthsPerCol)) / 12);
        g_date_set_dmy (start, 1, gdm, gdy);
        *end = *start;
...
month_coords (GncDenseCal *dcal, int monthOfCal, GList **outList)
{
    ...
    {
        auto gdm = static_cast<GDateMonth>((dcal->month - 1 + monthOffset) % 12 + 1);
        auto gdy = dcal->year + floor ((dcal->month - 1 + monthOffset) / 12);
        g_date_set_dmy (startD, 1, gdm, gdy);
    ...
    auto gdm = static_cast<GDateMonth>((dcal->month - 1 + monthOfCal) % 12 + 1);
    auto gdy = dcal->year + floor ((dcal->month - 1 + monthOfCal) / 12);
    g_date_set_dmy (startD, 1, gdm, gdy);
...
wheres_this (GncDenseCal *dcal, int x, int y)
{
    ...
    if (y >= static_cast<int>(dcal->day_top_bar_height + col_height (dcal)))
    {
        return -1;
    }
