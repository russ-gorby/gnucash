...
gnc_date_format_get_format (GNCDateFormat *gdf) noexcept
{
    ...
    return static_cast<QofDateFormat>(
...
gnc_date_format_refresh (GNCDateFormat *gdf) noexcept
{
    ...
    case QOF_DATE_FORMAT_UTC:
        format = g_strdup(qof_date_format_get_string(static_cast<QofDateFormat>(sel_option)));
    ...
    {
        format = g_strdup(
            qof_date_text_format_get_string(static_cast<QofDateFormat>(sel_option))
        );
        if (! gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gdf->months_number)))
        {
            if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(gdf->months_name)))
