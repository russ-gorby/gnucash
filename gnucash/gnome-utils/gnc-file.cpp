...
gnc_file_chooser_add_filters (GtkFileChooser* file_box, GList *filters) noexcept
{
    ...
    gtk_file_chooser_set_filter (file_box, static_cast<GtkFileFilter *>(filters->data));
    g_list_free (filters);
}
...
gnc_file_query_save (GtkWindow *parent, gboolean can_cancel) noexcept
{
    ...
    while (qof_book_session_not_saved(current_book))
    {
        ...
        /* This should be the same message as in gnc-main-window.cpp */
