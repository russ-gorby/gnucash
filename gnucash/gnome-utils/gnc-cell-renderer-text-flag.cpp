/**
 * gnc-cell-renderer-text-flag.cpp -- text cell renderer with flag.
 *
...
static void
gnc_cell_renderer_text_flag_class_init(GncCellRendererTextFlagClass *object)
{
    GObjectClass *object_class       = G_OBJECT_CLASS(object);
    GtkCellRendererClass *cell_class = GTK_CELL_RENDERER_CLASS(object);
                                         ...
                                         GParamSpec *pspec)
{
    ...
    case PROP_FLAG_COLOR_RGBA:
    {
        auto rgba = static_cast<GdkRGBA *>(g_value_get_boxed(value));
        if (rgba)
            celltext->color = *rgba;
    }
    break;
