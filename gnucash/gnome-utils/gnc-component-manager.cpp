...
dump_components (void)
{
    ...
    {
        auto ci = static_cast<ComponentInfo *>(node->data);
...
{
    auto et = static_cast<QofEventId *>(value);
...
{
    qof_string_cache_remove (static_cast<const char *>(key));
    g_free (value);
...
{
    auto guid = static_cast<GncGUID *>(key);
    auto ei = static_cast<EventInfo *>(value);
           ...
           QofEventId event_mask, gboolean or_in)
{
    ...
    if (event_mask == 0)
    {
        ...
        if (g_hash_table_lookup_extended (hash, entity, &key, &value))
        {
            ...
            guid_free (static_cast<GncGUID *>(key));
            g_free (value);
        }
    }
    else
                ...
                QofEventId event_mask, gboolean or_in)
{
    ...
    auto mask = static_cast<QofEventId *>(
        g_hash_table_lookup (cei->event_masks, entity_type)
    );
    if (!mask)
    {
        const char * key = qof_string_cache_insert (entity_type);
        mask = g_new0 (QofEventId, 1);
        g_hash_table_insert (cei->event_masks, const_cast<char *>(key), mask);
    }
...
gnc_gui_get_entity_events (GHashTable *changes, const GncGUID *entity)
{
    ...
    return static_cast<EventInfo *>(g_hash_table_lookup (changes, entity));
}
...
{
    auto cei = static_cast<ComponentEventInfo *>(user_data);
    auto id_type = static_cast<QofIdType>(key);
    auto et = static_cast<QofEventId *>(value);

    auto et_2 = static_cast<QofEventId *>(g_hash_table_lookup (cei->event_masks, id_type));
    if (!et_2)
        return;
...
{
    auto guid = static_cast<GncGUID *>(key);
    auto ei_1 = static_cast<EventInfo *>(value);
    auto cei = static_cast<ComponentEventInfo *>(user_data);

    auto ei_2 = static_cast<EventInfo *>(g_hash_table_lookup (cei->entity_events, guid));
    if (!ei_2)
        return;
...
gnc_close_gui_component_by_session (gpointer session)
{
    ...
    // reverse the list so class like dialog-options cpplose before window-report
    list = g_list_reverse (list);
