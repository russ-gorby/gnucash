...
gnc_item_edit_tb_new (GnucashSheet *sheet)
{
    ...
    GncItemEditTb *item_edit_tb = static_cast<GncItemEditTb *>(
...
draw_background_cb (GtkWidget *widget, cairo_t *cr, gpointer user_data)
{
    ...
    color_type = gnc_table_get_color (item_edit->sheet->table, item_edit->virt_loc, nullptr);
    gnucash_get_style_classes (item_edit->sheet, stylectxt,
        static_cast<RegisterColor>(color_type), FALSE);
...
gnc_item_edit_new (GnucashSheet *sheet)
{
    ...
    GtkBorder margin;
    GtkBorder border;
    GncItemEdit *item_edit = static_cast<GncItemEdit *>(
        g_object_new (GNC_TYPE_ITEM_EDIT,
                      "sheet", sheet,"spacing", 0,
                      "homogeneous", FALSE, nullptr));
